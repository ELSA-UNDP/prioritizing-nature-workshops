...
#' @export
#'
calc_feature_representation <- function(prob.all,
                                        elsa_result,
                                        elsa_raster,
                                        feat_df) {
  ...
  overall_rep <- prioritizr::eval_feature_representation_summary(prob.all, elsa_result) |>
    dplyr::rename(
      ...
      total_amount_overall = total_amount #absolute numbers of possible PUs across all zones for feature
    ) |>
  ...
  relative_rep <- prioritizr::eval_feature_representation_summary(prob.all, elsa_result) |>
    ...
    dplyr::mutate(relative_held_zone = absolute_held / total_amount_overall) |> #calculate the proportion of selected PUs for a feature in a zone vs all possible
    dplyr::select("zone", "feature", "relative_held_zone") |>
    tidyr::pivot_wider(names_from = zone,
                       values_from = relative_held_zone)
  ...
  feature_rep_tabl <- feature_rep |>
    dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~ round(. * 100, 3))) |>
    dplyr::rename_with( ~ tibble::as_tibble(cats(elsa_raster)[[1]])$label, .cols = -c(1:3)) |>
    dplyr::select(3:7) |>
    tibble::add_column(Name = feat_df$label,
                       Theme = feat_df$theme,
                       .before = 1) |> 
    dplyr::rename(
      "{get_translation('data')}" := Name,
      "{get_translation('theme')}" := Theme,
      "{get_translation('overall')}" := relative_held_overall,
    )
